...
if command -v "$BINARY_NAME" >/dev/null 2>&1; then
    success "outline-mcp installed successfully!"
    printf "\n"
    printf "  ${BOLD}Register with Claude Code (per project):${NC}\n"
    printf "    cd your-project\n"
    printf "    claude mcp add outline --scope project outline-mcp \\\\\n"
    printf "      --env OUTLINE_URL=https://outline.example.com \\\\\n"
    printf "      --env OUTLINE_API_KEY=ol_api_your_key\n"
    printf "\n"
    printf "  Verify in Claude Code with ${BOLD}/mcp${NC}\n"
    printf "\n"
else
